...
const Order = () => {
  ...
  const thu = new Date(film.date).getDay();
  // useEffect(() => {
      ...
      if (thu === 1 || thu === 3) {
        ...
        if (!enable.includes(e.target.innerHTML)) {
          if (!chair.vip) {
            ...
            setNumberChair((pre) => [...pre, e.target.innerHTML]);
            setPriceFilm(
              (prev) => +film.price + prev + parseInt(film.priceVip)
            );
            setDetailTicket((pre) => [
              ...pre,
              {
